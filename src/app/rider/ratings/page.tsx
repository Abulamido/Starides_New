...
export default function RiderRatingsPage() {
  ...
  // Fetch reviews for this rider
  const reviewsQuery = useMemoFirebase(() => {
    if (!firestore || !user) return null;
    return query(
      collection(firestore, 'riderReviews'),
          ...
          {reviews && reviews.length > 0 ? (
              ...
              {reviews.map((review) => (
                <div key={review.id} className="border-b pb-6 last:border-0 last:pb-0">
                  ...
                  <p className="text-xs text-muted-foreground mt-2">
                    Delivery #{review.orderId.substring(0, 8)}
                  </p>
                </div>
              ))}
            </div>
            ...
            <div className="flex flex-col items-center justify-center gap-4 text-center text-muted-foreground py-12">
              ...
              <div>
                ...
                <p className="text-sm">Your ratings and reviews will appear here once you complete deliveries.</p>
              </div>
            </div>
          )}
        </CardContent>
